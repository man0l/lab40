<x-layout>
    <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-gray-200 dark:border-gray-700">
            ...
            <form method="POST" action="{{ route('booking.update', $booking) }}" class="space-y-6">
                ...
                <div class="mb-5">
                    <label for="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Booking Description</label>
                    <div class="relative">
                        ...
                        <textarea id="description" name="description" rows="4" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Enter details about the appointment">{{ old('description', $booking->description) }}</textarea>
                    </div>
                    @error('description')
                        <div class="flex items-center mt-2 text-sm text-red-600 dark:text-red-500">
                            <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="mb-4">
                        <div class="flex items-center space-x-3 mb-4">
                            <div class="flex items-center">
                                <input id="customer-option-existing" type="radio" name="customer_option" value="existing" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" {{ old('customer_option') !== 'new' ? 'checked' : '' }} onchange="toggleCustomerFields(this)">
                                <label for="customer-option-existing" class="ml-2 text-sm font-medium text-gray-900 dark:text-white">Select existing customer</label>
                            </div>
                            <div class="flex items-center">
                                <input id="customer-option-new" type="radio" name="customer_option" value="new" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" {{ old('customer_option') === 'new' ? 'checked' : '' }} onchange="toggleCustomerFields(this)">
                                <label for="customer-option-new" class="ml-2 text-sm font-medium text-gray-900 dark:text-white">Create new customer</label>
                            </div>
                        </div>
                    </div>
                    
                    <!-- Existing customer dropdown -->
                    <div id="existing-customer-section" class="mb-4 {{ old('customer_option') === 'new' ? 'hidden' : '' }}">
                        <label for="customer_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select Customer</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <select id="customer_id" name="customer_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <option value="">Select a customer</option>
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->id }}" {{ old('customer_id', $booking->customer_id) == $customer->id ? 'selected' : '' }}>
                                        {{ $customer->firstname }} {{ $customer->lastname }} ({{ $customer->pin }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="relative">
                        ...
                        <select id="notification_channel_id" name="notification_channel_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                            @foreach($notificationChannels as $channel)
                                <option value="{{ $channel->id }}" {{ old('notification_channel_id', $booking->notification_channel_id) == $channel->id ? 'selected' : '' }}>{{ $channel->name }}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    <script>
        function toggleCustomerFields(radio) {
            ...
            if (radio.value === 'existing') {
                ...
                customerIdSelect.setAttribute('required', '');
            } else {
